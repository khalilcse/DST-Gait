...
class DSTGait(pl.LightningModule):
    def __init__(
            self,
            learning_rate: float = 0.001, #0.005
        ...
        super().__init__()
        self.train_loss_curve = [] # for verstio 2.0.0
        ...
        print('From DSTGait: Start Main Function----')
        self.backbone = StGCN(3, self.graph) # Define the base model for ST-GCN
        ...
        """
        self.distance = distances.CosineSimilarity()   #LpDistance() # cosign similarity
        self.train_loss = losses.SupConLoss(loss_temperature, distance=self.distance) # Constrctive Loss
    ...
    def training_step(self, batch, batch_idx):
        x, y, _ = batch
        #print(f"The feature size at Training Step:{x.shape}")
        y_hat = self(x)
        loss = self.train_loss(y_hat, y.squeeze())
        #print(f"At training: loss:{len(loss)} ")
        self.log('train_loss', loss, on_step=False, on_epoch=True, prog_bar=True, logger=True)
        return self.test_step(batch, batch_idx)

    def training_epoch_end(self, outputs):
        return self.test_epoch_end(outputs, print_output=False, state = 1) # outputs
    ...
    def test_epoch_end(self, outputs, print_output=True,state = 3): # State 3 for test 2 for validation 1 for training
        embeddings = dict()
        ...
        if state == 1:
            self.log("training/accuracy", accuracy_avg)
        elif state == 2:
            self.log("validation/accuracy", accuracy_avg)
        else:
            self.log("test/accuracy", accuracy_avg)
...
class CASIABPoseModule(pl.LightningDataModule):
    ...
    def sample_batch(self):
        # Create a DataLoader with batch_size=1 to obtain a sample batch
        dataloader1 = DataLoader(self.dataset_train, batch_size=2)
        sample_batch = next(iter(dataloader1))
        return sample_batch
    """
